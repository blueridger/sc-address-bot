...
var useListParams = function (_a) {
    ...
    var changeParams = react_1.useCallback(function (action) {
        if (!tempParams.current) {
            ...
            tempParams.current = queryReducer_1.default(query, action);
            // schedule side effects for next tick
            setTimeout(function () {
                if (syncWithLocation) {
                    history.push({
                        search: "?" + query_string_1.stringify(__assign(__assign({}, tempParams.current), { filter: JSON.stringify(tempParams.current.filter), displayedFilters: JSON.stringify(tempParams.current.displayedFilters) })),
            ...
            tempParams.current = queryReducer_1.default(tempParams.current, action);
        }
    }, requestSignature); // eslint-disable-line react-hooks/exhaustive-deps
